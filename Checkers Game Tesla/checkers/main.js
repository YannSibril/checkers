...
//Initialize the Checkers board creating Tile objects , flipping colors 
function init(){
...
//Collision detection function to check if two objects are  colliding together, 
// If any of the conditions are true the objects are not colliding 
...
// Event handler function with argument e to  check if AI is making a move, using getPos to retrieve mouse position 
...
// Event conditions to check if the pieces movement is valid or not     
...
//Updates the positions of pieces on the board after a move has been made or not.
function updatePiecesPos(piecemoving, move, i, bf){
    ...
    switch(move[1])
    {
        case MOVE_TYPE.MOVE_NORMAL:
        {
            ...
            updateSuperPiece(bf.bPieces, PIECE_TYPE.SUPER_BLACK);
...
// Updates the piece type to a queen iterating through the tiles. 
...
// Event handle for mouse down , checking if the AI is maling a move or not 
...
// Updates the position of moving piece to follow mouse 
...
// Auxiliary function to get the position
function getPos(c, e){
...
//Function to repeatedly update and draw the game state
function loop() {
    if(DEBUG){
        zz.innerHTML = "";
    ...
    draw();
 // Conditions to alert wether user win or computer wins
    if(BOARD_DEF.availableMoves.length <= 0){
        counterAlert++;
    }
    if(counterAlert > 30){
        let p = BOARD_DEF.move == PLAYER.P1 ? "You" : "Computer";
        stopLoop = true;
        alert(p +" win!");
    }
    if(!stopLoop)
